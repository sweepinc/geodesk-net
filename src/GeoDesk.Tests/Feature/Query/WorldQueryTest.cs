    ...
    public void AllFeaturesAreWithinMonacoBounds()
    {
        ...
        try
        {
            ...
            foreach (var node in world.Nodes())
            {
                ...
                if (lon < actualMinLon) actualMinLon = lon;
                if (lon > actualMaxLon) actualMaxLon = lon;
                if (lat < actualMinLat) actualMinLat = lat;
                if (lat > actualMaxLat) actualMaxLat = lat;
            }
            ...
            Assert.True(actualMinLon >= expectedMinLon && actualMinLon <= monacoExpectedCenterLon,
                $"Minimum longitude {actualMinLon:F6} is outside expected range for Monaco region");
            Assert.True(actualMaxLon <= expectedMaxLon && actualMaxLon >= monacoExpectedCenterLon,
                $"Maximum longitude {actualMaxLon:F6} is outside expected range for Monaco region");
            Assert.True(actualMinLat >= expectedMinLat && actualMinLat <= monacoExpectedCenterLat,
                $"Minimum latitude {actualMinLat:F6} is outside expected range for Monaco region");
            Assert.True(actualMaxLat <= expectedMaxLat && actualMaxLat >= monacoExpectedCenterLat,
                $"Maximum latitude {actualMaxLat:F6} is outside expected range for Monaco region");
